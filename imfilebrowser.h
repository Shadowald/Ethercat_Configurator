...
inline void ImGui::FileBrowser::Display()
{
    ...
    for (const auto& sec : pwd_)
    {
        ...
        //Sets the size of the buttons, so that they are big enough for a touch screen and match the size of the frame padding of other elements in y
        const ImVec2 label_size = CalcTextSize(u8StrToStr(sec.u8string()).c_str(), NULL, true);
        ImGuiStyle& style = ImGui::GetStyle();
        float buttonSizeX;
        if (label_size.x + style.FramePadding.x * 2.0f < 33)
            buttonSizeX = 33;
        else
            buttonSizeX = label_size.x + style.FramePadding.x * 2.0f;
        ...
        if (Button(u8StrToStr(sec.u8string()).c_str(), ImVec2(buttonSizeX, 33)))
        {
